...
local isWithdrawing = false  -- متغير لتحديد ما إذا كانت عملية السحب جارية

Tab5:AddToggle({
    Name = "💰سحب جميع الوقت (تلقائي)",
    Default = false,
    Callback = function(value)
        isWithdrawing = value  -- تحديد ما إذا كان الزر مفعل أم لا
        while isWithdrawing do
            ...
            -- الانتظار قليلاً قبل السحب مرة أخرى (لتقليل الضغط على الخادم)
            wait(1)  -- يمكن تعديل الوقت هنا بحسب ما ترغب
        end
    end
})
...
Tab:AddToggle({
    ...
    Callback = function(Value)
        ...
        local character = player.Character or player.CharacterAdded:Wait()
        local rootPart = character:WaitForChild("HumanoidRootPart")
        local originalPosition = nil  -- سيتم تحديد الموقع عند الانتقال لأول مرة

        -- دالة للبحث عن EventCoin
        local function searchForEventCoin()
            local eventCoin = workspace:FindFirstChild("EventCoin")
            if eventCoin then
                -- حفظ الموقع الأصلي قبل الانتقال
                if not originalPosition then
                ...
                -- الانتقال إلى EventCoin
                rootPart.CFrame = eventCoin.CFrame

                -- الانتظار حتى تختفي العملة
                repeat wait(0.1) until not workspace:FindFirstChild("EventCoin")

                -- العودة إلى الموقع الأصلي
                if originalPosition then
                    rootPart.CFrame = CFrame.new(originalPosition)
                    originalPosition = nil -- إعادة التهيئة للبحث القادم
                end
            end
        end

        -- تكرار البحث طالما أن الزر مفعّل
        while isRunning do
            searchForEventCoin()
            wait(0.1) -- تقليل الضغط على السيرفر
        end
    end
})
